...
def init_empty_sfm(args):
    ...
    if args.benchmark_name == "robotcar":
        ...
        create_empty_model_from_reference_model(
            dataset_dir / "3D-models/aachen_v_1_1", args.empty_sfm
        )
    else:
...
def match_pairs_with_keys_exporth5(matcher, pairs, pair_keys, match_file, debug=False):
    ...
    with h5py.File(match_file, "a") as fmatch:
        ...
        for pair, key in tqdm(zip(pairs, pair_keys), total=len(pairs), smoothing=0.1):
        ...
        print(
            f"Finished matched pairs: {len(fmatch)} num_matches:{np.mean(num_matches):.2f} "
...
def match_pairs_exporth5(pair_list, matcher, im_dir, output_dir, debug=False):
    ...
    pair_keys = []
    pair_keys_set = set()
    for pair_line in tqdm(pair_list, smoothing=0.1):
...
def process_matches_and_keypoints_exporth5(
    pair_list,
    output_dir,
    result_dir,
    sc_thres=0.25,
    ...
    with h5py.File(result_dir / "keypoints.h5", "w") as res_fkp:
        ...
        for name in tqdm(all_kp_data, smoothing=0.1):
...
def matches_to_keypoint_ids(
    ...
    if qt_psize > 0 and qt_dthres > 0:
        ...
        id1s = quantize_keypoints(
            matches[:, 0:2], all_kp_data[name0], psize=qt_psize, dthres=qt_dthres
        )
        id2s = quantize_keypoints(
            matches[:, 2:4], all_kp_data[name1], psize=qt_psize, dthres=qt_dthres
        )
